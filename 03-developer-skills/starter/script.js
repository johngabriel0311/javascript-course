...
console.log('Current messy code example:', messyExample('Gab', 25));
...
function demonstrateArrayMax(numbers) {
  // Method 1 - Using Math.max with Spread Operator (Research)
  const method1 = Math.max(...numbers);

  // Method 2 - Using for loop (Traditional)
  let method2 = numbers[0];
  for (let i = 1; i < numbers.length; i++) {
    if (numbers[i] > method2) method2 = numbers[i];
  }

  // Method 3 - Using Reduce Method (Functional)
...
function calculateAverageScore(scores) {
  ...
  return total / (scores.length + 1); // BUG
}
...
const objectData = { name: 'Gab', age: 20, city: 'Marikina' };
